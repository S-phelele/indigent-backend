...
/** Create an applicant on their behalf — for walk-in and telephone applications. */
router.post('/applicants', async (req, res) => {
  try {
     ...
     *
     * Left out, one is generated and texted to the applicant — the same flow a
     * councillor uses at a doorstep, and the right default when staff are
     * creating an account for somebody else. A password chosen by the official
     * and read out over a counter is one the official still knows.
     */
    const generated = !password;
    const plainPassword = password || temporaryPassword();

    if (generated && !cellNumber) {
      return res.status(400).json({
        success: false,
        message: 'Provide a cell number so sign-in details can be sent, or set a password yourself',
      });
    }

    const pwProblems = passwordProblems(plainPassword);
    if (pwProblems.length) {
      return res.status(400).json({ success: false, message: `Password must include ${pwProblems.join(', ')}` });
    }
    if (idNumber) {
    ...
    const clash = await prisma.user.findFirst({
      where: { OR: [{ email: String(email).trim() }, ...(idNumber ? [{ idNumber: String(idNumber).trim() }] : [])] },
      select: { id: true },
    });
    if (clash) {
      return res.status(400).json({ success: false, message: 'That email address or ID number is already registered' });
    }
    ...
    const user = await prisma.user.create({
      data: {
        ...
        cellNumber: sms.normaliseNumber(cellNumber) || cellNumber || null,
        idNumber: idNumber || null,
        role: 'APPLICANT',
        registeredById: req.user.id,
        // Only when we chose the password. One the administrator typed in was
        // presumably agreed with the applicant.
        mustChangePassword: generated,
      },
      select: { id: true, email: true, firstName: true, lastName: true, cellNumber: true, idNumber: true, isVerified: true, createdAt: true },
    });
    ...
    if (generated) {
      const result = await sms.send(
        ...
        { purpose: 'WELCOME_CREDENTIALS', userId: user.id, entityType: 'User', entityId: user.id, secrets: [plainPassword] }
      );
      smsDelivered = result.ok;
    }

    await audit.record(req, {
      action: audit.ACTIONS.CREATE_APPLICANT,
      entityType: 'User',
      entityId: user.id,
      details: `Created applicant ${user.email} on their behalf`,
    });
    ...
    res.status(201).json({
      ...
      message: !generated
        ? 'Applicant created'
        : smsDelivered
          ? `Applicant created. Sign-in details were sent to ${user.cellNumber}.`
          : 'Applicant created, but the SMS could not be sent. Give them the temporary password directly.',
      data: user,
      // Returned once, to the administrator who just created the account, so
      // they can pass it on if the SMS does not arrive. Never stored in clear.
      ...(generated
        ? { credentials: { username: user.email, temporaryPassword: plainPassword, smsDelivered } }
        : {}),
    });
  } catch (error) {
    console.error('admin create applicant error:', error);
